<template>
  ...
  <div class="pt-5 height-top">
    <div class="col-12 d-flex justify-content-center pt-5">
      ...
      <button class="rounded text-shadow-dark mx-3" data-bs-toggle="modal" data-bs-target="#mbt-odds"
    ...
    <div class="col-12 d-flex justify-content-center gap-3">
      <div class="d-flex">
        <input id="highlands" class="mx-1 checkbox" type="checkbox" v-model="highlands"
          :disabled="grassland || forest || desert || urban">
        <h3>Highlands</h3>
      </div>
      <div class="d-flex">
        <input id="grassland" class="mx-1 checkbox" type="checkbox" v-model="grassland"
          :disabled="highlands || forest || desert || urban">
        <h3>Grassland</h3>
      </div>
    </div>
    <div class="col-12 d-flex text-center justify-content-center gap-3">
      <div class="d-flex">
        <input id="forest" class="mx-1 checkbox" type="checkbox" v-model="forest"
          :disabled="grassland || highlands || desert || urban">
        <h3>Forest</h3>
      </div>
      <div class="d-flex">
        <input id="desert" class="mx-1 checkbox" type="checkbox" v-model="desert"
          :disabled="grassland || forest || highlands || urban">
        <h3>Desert</h3>
      </div>
    </div>
    <div class="col-12 d-flex justify-content-center gap-3">
      <input id="urban" class="mx-1 checkbox" type="checkbox" v-model="urban"
        :disabled="grassland || forest || desert || highlands">
      <h3>Urban</h3>
    </div>

    <!-- Defense dice-->
    <div class="d-flex justify-content-center">
      <div class="col-3 d-flex justify-content-center mt-5" v-if="highlands || grassland || urban || desert || forest">
        <div class="text-center">
          <h6>Roll Dice</h6>
...
function offensiveDie() {
  ...
  if (infantry.value === true) {
    if ([2, 7, 8, 9, 10, 11, 12].includes(twoSixDie.value.roll)) {
      success.value = true
    } else {
      success.value = false
    }
  ...
  // vs ifv
  if (ifv.value === true) {
    if ([5, 6, 7, 8, 9, 10, 11, 12].includes(twoSixDie.value.roll)) {
      success.value = true
    } else {
      success.value = false
    }
